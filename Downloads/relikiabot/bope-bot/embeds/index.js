...
const RODAPE = 'Cidade Alta — Facção — Cidade Alta | FiveM RP';

// ── REGRAS GERAIS (Discord) ───────────────────────────────────────────────────
export function embedRegras() {
  return new EmbedBuilder()
    .setColor(COR_FAC)
    .setTitle('📋 REGULAMENTO OFICIAL — CIDADE ALTA')
    .setDescription('Leia com atenção. O descumprimento das regras resulta em punição ou exclusão da facção.')
    .addFields(
      {
        ...
        value: [
          ...
          '> **1.5** Proibido expor informações internas da facção fora do servidor.',
        ].join('\n')
      },
      {
        name: '🎮 2. ROLEPLAY',
        value: [
          ...
          '> **2.4** Qualquer ação da facção deve ter base em roleplay — sem ataque gratuito.',
          '> **2.5** O uniforme e identificação da Cidade Alta devem ser usados apenas em atividade.',
        ].join('\n')
      },
      {
      ...
      },
      {
        ...
        value: [
          ...
          '> **4.2** Proibido fazer parte de organizações rivais enquanto membro ativo da Cidade Alta.',
...
export function embedRegrasRP() {
  return new EmbedBuilder()
    .setColor(COR_INFO)
    .setTitle('🎭 REGRAS DE ROLEPLAY — CIDADE ALTA')
    .setDescription('Regras específicas de RP para membros da Cidade Alta dentro do servidor FiveM.')
    .addFields(
      {
        name: '📖 1. BÁSICO DO RP',
        value: [
      ...
      },
      {
        ...
        value: [
          ...
          '> **4.3** Proibido se passar por membro de outra facção (false flag).',
        ].join('\n')
      },
      {
...
export function embedSobre() {
  return new EmbedBuilder()
    .setColor(COR_FAC)
    .setTitle('🔴 SOBRE A CIDADE ALTA')
    .setDescription('**Cidade Alta — Facção — Cidade Alta**\nNascida nas favelas, dominando as ruas. A Cidade Alta é uma facção mista que atua com força, estratégia e lealdade.')
    .addFields(
      {
        name: '📌 Nossa Missão',
        value: '> Dominar e proteger nosso território com disciplina, força e lealdade. A Cidade Alta não é uma gangue qualquer — é uma facção estruturada com hierarquia e respeito.'
      },
      {
        name: '💀 Especialidades',
        value: [
...
export function embedRequisitos() {
  return new EmbedBuilder()
    .setColor(COR_INFO)
    .setTitle('🎯 REQUISITOS PARA ENTRAR NA CIDADE ALTA')
    ...
    .addFields(
      ...
      },
      {
        ...
        value: [
          ...
          '> 🤝 Indicação de um membro ativo da Cidade Alta',
        ].join('\n')
      },
      {
        name: '❌ Impedimentos',
...
export function embedSuporte() {
  const embed = new EmbedBuilder()
    .setColor(COR_INFO)
    .setTitle('🛠️ SUPORTE — CIDADE ALTA')
...
export function embedCandidatura() {
  const embed = new EmbedBuilder()
    .setColor(COR_FAC)
    .setTitle('📝 CANDIDATURA — CIDADE ALTA')
    .setDescription('Quer fazer parte da facção? Clique no botão abaixo e inicie sua candidatura.\nUm canal privado será aberto para você responder as perguntas.')
    .addFields(
      {
...
export function embedCandidaturaAberta(usuario) {
  const embed = new EmbedBuilder()
    .setColor(COR_FAC)
    .setTitle('📝 CANDIDATURA INICIADA — CIDADE ALTA')
    ...
    .addFields(
      ...
      },
      {
        ...
        value: [
          ...
          '> **5.** Já fez parte de alguma facção ou facção? Qual?',
          '> **6.** Por que quer entrar na Cidade Alta?',
...
export function embedAprovado(usuario, nomeJogo, idJogo, idadeJogo) {
  return new EmbedBuilder()
    ...
    .setDescription(`${usuario} foi aprovado e agora faz parte da Cidade Alta!\nBem-vindo à facção. 🔴`)
    .addFields(
