...
export async function handleInteraction(interaction, client) {
  ...
  if (customId === 'aprovar_candidatura') {
    ...
    await usuarioAlvo.send(`✅ **Parabéns! Sua candidatura à Cidade Alta foi APROVADA!**\nBem-vindo à facção. 🔴\n\n🎮 **Personagem:** ${nomeJogo}\n🪪 **ID:** ${idJogo}`).catch(() => {});
  ...
  if (customId === 'reprovar_candidatura') {
    ...
    await usuarioAlvo.send('❌ **Sua candidatura à Cidade Alta foi reprovada.**\nTente novamente após 7 dias.').catch(() => {});
