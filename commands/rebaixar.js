...
const CARGOS_BOPE = ['Coronel', 'Cabo', 'Soldado'];

export default {
  data: new SlashCommandBuilder()
    .setName('rebaixar')
    .setDescription('Remove o cargo BOPE de um membro')
  ...
  async execute(interaction, { enviarLog }) {
    ...
    for (const nomeCargo of CARGOS_BOPE) {
    ...
    if (cargosRemovidos.length === 0) {
      return interaction.reply({ content: '❌ Este membro não possui nenhum cargo do BOPE.', ephemeral: true });
    }
